...
import { Loader2, Copy, CheckCircle2, Mail, Send, Sparkles } from "lucide-react";
...
export function EmailPitchDialog({ 
  ...
  const handleGenerate = () => {
    if (!context.trim()) {
      toast({ title: "Required", description: "Please provide customer context.", variant: "destructive" });
      return;
    }
  ...
  const handleCopy = (text: string, type: 'subject' | 'body') => {
    ...
    toast({ title: "Copied", description: `${type === 'subject' ? 'Subject' : 'Email body'} copied to clipboard.` });
  };
      ...
      <DialogContent className="sm:max-w-[650px] flex flex-col max-h-[90vh]">
        <DialogHeader className="pb-2 border-b">
          ...
          <DialogDescription className="pt-1">
            Personalize the outreach for <span className="font-semibold text-foreground">{ipName}</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        {!generatePitch.data ? (
          <div className="space-y-5 py-4">
            <div className="space-y-3">
              <Label htmlFor="context" className="text-sm font-semibold flex items-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                This context will guide the AI to focus on the specific pain points mentioned.
              </p>
            </div>
          </div>
        ) : (
          <div className="space-y-5 py-4 overflow-y-auto pr-2">
            <div className="space-y-2">
              <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Subject Line</Label>
              <div className="flex gap-2">
                ...
                <Button size="icon" variant="outline" onClick={() => handleCopy(generatePitch.data.subject, 'subject')}>
                  {copiedSubject ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                </Button>
              </div>
            </div>
            
            <div className="space-y-2">
              <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Email Body</Label>
              <div className="relative group">
                ...
                >
                  {copiedBody ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                </Button>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="border-t pt-4 mt-auto">
          {!generatePitch.data ? (
            ...
            <div className="flex justify-between w-full">
              ...
              <Button onClick={() => onOpenChange(false)} className="font-semibold">
                Done
              </Button>
            </div>
          )}
