...
import {
  ...
  BarChart3, Search, Upload, ArrowRight, CheckCircle2, Users, TrendingUp,
} from "lucide-react";
...
function Feature({ icon, color, title, description }: {
      ...
      <main className="flex-1 container mx-auto max-w-4xl p-6 md:p-10 space-y-14">
        ...
        <section className="text-center space-y-5 pt-4">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight text-foreground leading-tight">
            What is <span className="text-primary">Smart IP Advisor</span>?
          </h1>
          <p className="text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">
            Smart IP Advisor is an AI-powered tool that helps SAP sales and consulting teams instantly surface the most relevant IP solutions from customer conversations — turning meeting transcripts into targeted recommendations and ready-to-send pitches.
          </p>
        </section>

        {/* ── The Problem ── */}
        <section className="bg-white border border-border rounded-2xl p-8 shadow-sm space-y-4">
          <div className="flex items-center gap-2 mb-2">
            <Target className="w-5 h-5 text-rose-600" />
            <h2 className="text-xl font-bold text-foreground">The Problem It Solves</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            {[
              { icon: <Users className="w-4 h-4 text-rose-600" />, bg: "bg-rose-50", text: "Knowledge gaps between sales reps and the full IP library" },
              { icon: <TrendingUp className="w-4 h-4 text-amber-600" />, bg: "bg-amber-50", text: "Slow, manual process of matching customer needs to solutions" },
              { icon: <Mail className="w-4 h-4 text-blue-600" />, bg: "bg-blue-50", text: "Generic pitches that don't speak to the customer's specific challenges" },
            ].map(({ icon, bg, text }) => (
              <div key={text} className={`${bg} rounded-xl p-4 text-sm text-foreground/80 leading-relaxed flex gap-2`}>
        ...
        <section className="space-y-2">
          ...
          <div>
            <Step number={1} icon={<FileText className="w-4 h-4" />}
            ...
            <Step number={2} icon={<Search className="w-4 h-4" />}
            ...
            <Step number={3} icon={<BarChart3 className="w-4 h-4" />}
            ...
            <Step number={4} icon={<Lightbulb className="w-4 h-4" />}
            ...
            <Step number={5} icon={<Mail className="w-4 h-4" />}
        ...
        {/* ── Who Is It For ── */}
        <section className="bg-gradient-to-br from-primary/5 to-blue-50/60 border border-primary/10 rounded-2xl p-8 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { role: "SAP Account Executives", desc: "Quickly identify which IPs fit your customer conversations without needing deep technical knowledge of the full catalog." },
              { role: "Pre-Sales Consultants", desc: "Accelerate solution matching after discovery workshops and produce polished customer pitches in minutes instead of hours." },
              { role: "Delivery Consultants", desc: "Keep the IP library current by adding new solutions, attaching technical documents, and refining entries as the portfolio evolves." },
            ].map(({ role, desc }) => (
              <div key={role} className="bg-white border border-border rounded-xl p-4 shadow-sm space-y-1.5">
