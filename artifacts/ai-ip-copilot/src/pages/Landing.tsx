...
import {
  FileText, Lightbulb, Target, Zap, Database, Mail,
  BarChart3, Search, Upload, ArrowRight, CheckCircle2,
...
const FEATURES = [
  ...
  },
  {
    ...
    description: "Each recommendation comes with a match score and color-coded rating (Strong / Good / Possible Match) so you know where to focus first.",
  },
  {
...
const PROBLEMS = [
  { icon: <Users className="w-4 h-4 text-rose-600" />, bg: "bg-rose-50 border-rose-100", text: "Knowledge gaps between sales reps and the full IP library" },
  { icon: <TrendingUp className="w-4 h-4 text-amber-600" />, bg: "bg-amber-50 border-amber-100", text: "Slow, manual process of matching customer needs to solutions" },
  { icon: <Mail className="w-4 h-4 text-blue-600" />, bg: "bg-blue-50 border-blue-100", text: "Generic pitches that don't speak to the customer's specific challenges" },
];
...
export default function Landing() {
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* ── Hero ── */}
      <section className="relative overflow-hidden bg-gradient-to-br from-primary/5 via-background to-blue-50/40 border-b border-border">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(var(--primary)/0.08),transparent)]" />
        <div className="relative container mx-auto max-w-5xl px-6 py-20 md:py-28 text-center space-y-7">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight text-foreground leading-tight">
            ...
            <span className="text-primary">targeted SAP IP pitches</span>
          </h1>
          <p className="text-lg md:text-xl text-muted-foreground leading-relaxed max-w-2xl mx-auto">
            Smart IP Advisor analyzes meeting transcripts with AI and instantly surfaces the most relevant SAP IP solutions — with personalized reasoning and ready-to-send emails.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            ...
            <Button size="lg" variant="outline" className="text-base px-7 h-12" asChild>
              <Link href="/catalog">
                Browse IP Catalog
              </Link>
            </Button>
      ...
      <section className="container mx-auto max-w-5xl px-6 py-16 space-y-8">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-black text-foreground">The Problem It Solves</h2>
        </div>
        <p className="text-muted-foreground leading-relaxed max-w-3xl">
          SAP has a large library of pre-built IP solutions, accelerators, and extensions — but connecting the right IP to the right customer at the right moment is hard. Sales teams often miss opportunities because they don't know which IP applies, or they spend hours manually reviewing solution catalogs after every discovery call.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {PROBLEMS.map(({ icon, bg, text }) => (
            <div key={text} className={`${bg} border rounded-xl p-5 text-sm text-foreground/80 leading-relaxed flex gap-3`}>
          ...
          <div className="grid md:grid-cols-2 gap-x-16">
            ...
            <div className="hidden md:flex items-center justify-center">
              <div className="bg-card border border-border rounded-2xl p-8 shadow-sm space-y-4 w-full max-w-xs">
                <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Example output</div>
                ...
                <div className="pt-2 space-y-2">
                  {["Strong Match · 87%", "Good Match · 72%", "Possible Match · 54%"].map((label, i) => (
                    <div key={i} className={`rounded-lg px-3 py-2 text-xs font-semibold ${i === 0 ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : i === 1 ? "bg-blue-50 text-blue-700 border border-blue-200" : "bg-amber-50 text-amber-700 border border-amber-200"}`}>
                      {label}
                    </div>
                  ))}
                </div>
      ...
      <section className="container mx-auto max-w-5xl px-6 py-16 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map(({ icon, color, title, description }) => (
            <div key={title} className="bg-card border border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      {/* ── Who Is It For ── */}
      <section className="bg-gradient-to-br from-primary/5 to-blue-50/60 border-y border-primary/10">
        <div className="container mx-auto max-w-5xl px-6 py-16 space-y-8">
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 text-primary" />
            <h2 className="text-2xl font-black text-foreground">Who Is It For?</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {PERSONAS.map(({ role, desc }) => (
              <div key={role} className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
      ...
      <section className="container mx-auto max-w-5xl px-6 py-20 text-center space-y-6">
        ...
        <p className="text-muted-foreground text-lg max-w-xl mx-auto">
          Paste your first meeting transcript and get AI-powered IP recommendations in seconds.
        </p>
